...
available from http://www.bioconductor.org and its 
methods are presented in \cite{jrg1}. The software is provided with a graphical user interface (GUI) that facilitates 
its use, in particular, for those not familiar with R. Here we present a step-by-step 
analysis of the data studied in reference \cite{jrg1}. }

\section{Installation}
MLPA stats is written for R 2.9.0, which itself can be downloaded from the R website http://www.r$-$project.org.
MLPA software in turn can be obtained from  http://www.creal.cat/jrgonzalez/software.html. The package is installed using:

\begin{verbatim}
> install.packages("MLPAstats_0.5-5.tar.gz",repos=NULL) 
\end{verbatim}
...
Nine of the probes are used as control probes for the normalization step.
...
Ms$mlpa$arg.mlpa$test.dat<-BRCAtests
Ms$mlpa$arg.mlpa$size.dat<-size
Ms$mlpa$arg.mlpa$probe.dat<-probes.control
@
...
text files; figure(\ref{Ms}). The option ``control and test data'' directs you to select the data 
files for the control and test samples. MLPAstats package provides samples of such files for the BRCA data. 
For the this example, you can select the option  and follow the dialog to browse on your local directory 
for paper.control.text and paper.test.txt. 
...
...
\end{verbatim}
where the first row is the sample, replica and probe names and data is entered 
separated by single spaces. Make sure your own files follow this convention; in particualr,
have in mind reserving the first two columns for sample names and replicas, 
even if you do not have any.  

Now you need to specify which of these probes are controls. They can be provided 
...
displays the variable names to be selected as control probes.

\begin{figure}[h]
\begin{picture}(150,200)(0,0)
\put(70,-40){{\resizebox{7cm}{!}{\includegraphics{figures/picture4.pdf}}}}
\end{picture}
\caption{option to select control probes.}
\label{sel1}
\end{figure}

For BRCA data, click on the names that correspond to the control probes (those starting with C) and 
...
If the control probes are entered from a text file, a list of probes numbers
corresponding to the controls should be written on a row separated by single spaces. 
A sample of such file is control.probes.txt, that can be selected using the ``from file'' 
option.  
...
peak intensities by the total sum of only the intensity of the control probes. 
...
normalization for each individual test sample. In Windows environment the figure is saved on the clipboard 
...
Once the data is normalized the inference of the dosage ratios between test and 
...
in copy number. Here the ratio between the control and test samples is calculated 
for each probe and assessed against a gain or lose thresholds (0.7, 1.33).  

\item \emph{REX-MLPA}: The Regression-Enhanced MLPA computes the regression between 
tests and controls with $(\alpha -1)$ confidence intervals. Tests probes with altered 
copy number are identified as outliers of the confidence region. Starting with the 
regression of only control probes the method iteratively includes the probes of the 
sample test that are within the confidence intervals to re-estimate the regression. 
...
A probe has a different copy number for a tests sample if their
...
analysis produces a scattered plot of test against control
...
If you are comfortable with R the previous analysis can be completed with a 
handful of steps. First, load the BRCA data with
<<>>=
data(BRCA)
@
and set it up as an object of class setupMLPA
<<>>=
mlpa.dat<-setupMLPA(BRCAcontrols,BRCAtests,size,probes.control)
@.
...
plot(ans)
@
You can check the final copy number assignment for each probe and test sample
<<>>=
ans
@ 
...
the GUI offers an easy way quickly try different analysis scenarios and recover 
